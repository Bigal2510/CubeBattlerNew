...
namespace Unity.Multiplayer.Center.NetcodeForEntitiesSetup
{
    ...
    public partial struct GoInGameServerSystem : ISystem 
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            var spawnPointsLtWs = spawnPointsQuery.ToComponentDataArray<LocalToWorld>(Allocator.Temp);
            var spawnPT = GetSpawnPoint(spawnPointsLtWs);
        ...
        private float3 GetSpawnPoint(NativeArray<LocalToWorld> spawnPointsLtWs)
        {
            int rngIndex = UnityEngine.Random.Range(0, spawnPointsLtWs.Length);
            Debug.Log("SPAWNING AT POS: " + rngIndex);
            return spawnPointsLtWs[rngIndex].Position;
        }
    }
}
